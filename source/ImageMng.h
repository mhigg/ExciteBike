...
struct ActionHeader {
	...
	bool loop;				// ﾙｰﾌﾟするかどうか
	int animCnt;			// ｱﾆﾒｰｼｮﾝのｺﾏ数
};
...
private:
	...
	std::map<std::string, ImageHeader> imageHeader;		// 画像のﾍｯﾀﾞｰ
	std::map<std::string, ActionHeader> actHeader;		// ｱｸｼｮﾝﾍｯﾀﾞｰ
	MAP_STR_ACT actData;	// ｱｸｼｮﾝﾃﾞｰﾀ
};